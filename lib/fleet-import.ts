...
import { DEFAULT_CAB_TYPE, DEFAULT_FLEET_TYPE, type TrailerType } from "./types";
  ...
  for (const row of rows) {
    ...
    if (matchTruckId) {
      ...
      updateTruck(existing.id, {
        ...
        vin: preferFilled(row.vin, existing.vin),
        plate: preferFilled(row.plate, existing.plate),
        year: preferFilled(row.year, existing.year),
        make: preferFilled(row.make, existing.make),
        model: preferFilled(row.model, existing.model),
