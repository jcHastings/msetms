...
import { LOAD_MAP_PIN_SIZE, type LoadMapPoint } from "./load-map-shared";
...
export function clusterPinSvg(count: number): string {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${LOAD_MAP_PIN_SIZE}" height="${LOAD_MAP_PIN_SIZE}" viewBox="0 0 28 28"><circle cx="14" cy="14" r="12" fill="#0b1f3a" stroke="#d4a017" stroke-width="2"/><text x="14" y="18" text-anchor="middle" fill="#ffffff" font-size="10" font-weight="700" font-family="system-ui,sans-serif">${label}</text></svg>`;
}
