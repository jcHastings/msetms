...
export default async function DashboardPage({
  ...
  return (
    <PageOverlayHost returnTo={overlayReturnTo("/desk", current)} serverOpenId={openId}>
      <PageHeader
        title="Dispatch desk"
        actions={
            ...
            <ul className="mt-3 space-y-2 text-sm">
              {watched.map((load) => (
                <li key={load.id} className={`flex justify-between gap-2 ${loadStatusRowClass(load.status)} px-2 py-1`}>
                  <OverlayOpenLink
                    href={overlayHref("/desk", load.id, current)}
                ...
                {unassigned.map((load) => (
                  <tr key={load.id} className={loadStatusRowClass(load.status)}>
                    ...
                    <td className="text-right">
                      <OverlayOpenLink href={overlayHref("/desk", load.id, current)} className="btn btn-ghost">
                        Open
                      </OverlayOpenLink>
                    </td>
                  </tr>
      ...
      {openId ? <LoadOverlay loadId={openId} returnTo={overlayReturnTo("/desk", current)} /> : null}
    </PageOverlayHost>
  );
}
