...
async function main() {
  ...
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/backhaul-finder.tsx"), "utf8"), /data-backhaul-finder/);
  const backhaulUi = fs.readFileSync(path.join(process.cwd(), "components/backhaul-finder.tsx"), "utf8");
  const backhaulCopy = fs.readFileSync(path.join(process.cwd(), "lib/backhaul-shared.ts"), "utf8");
  assert.match(backhaulCopy, /No loads within 150 mi/);
  assert.match(backhaulCopy, /Need a delivery location/);
  ...
  assert.match(cssSource, /--glass: rgba\(255, 255, 255, 0\.72\)/);
  assert.match(cssSource, /--glass-border: rgba\(255, 255, 255, 0\.95\)/);
  assert.match(cssSource, /--glass-blur: 16px/);
  assert.match(cssSource, /--glass-shadow: 0 8px 28px rgba\(7, 50, 90, 0\.08\)/);
  assert.match(cssSource, /--canvas-wash: linear-gradient\(165deg, #c9daf0 0%, #e8eef5 42%, #f0f3f7 100%\)/);
  assert.match(cssSource, /\.desk-canvas \{[\s\S]*background: var\(--canvas-wash\)/);
  assert.match(cssSource, /\.glass-panel \{[\s\S]*backdrop-filter: blur\(var\(--glass-blur\)\) saturate\(var\(--glass-saturate\)\)/);
  assert.match(cssSource, /\.card \{[\s\S]*background: var\(--glass\)/);
  assert.match(cssSource, /\.workbench-card \{[\s\S]*background: var\(--glass\)/);
  assert.match(cssSource, /\.workbench-map-pane \{[\s\S]*background: #dce6ef/);
  assert.match(cssSource, /\.login-canvas \{[\s\S]*background: var\(--sidebar\)/);
  assert.match(cssSource, /\.desk-sidebar \{[\s\S]*background: var\(--sidebar\)/);
  assert.doesNotMatch(cssSource, /\.desk-sidebar \{[^}]*backdrop-filter/);
  assert.match(cssSource, /\.status-pill \{[\s\S]*border-radius: var\(--r-xs\)/);
  assert.match(cssSource, /\.btn-primary \{[\s\S]*background: var\(--accent\)/);
  assert.match(cssSource, /\.table-grid td \{[\s\S]*background: #fff/);
  assert.match(cssSource, /@supports not \(\(backdrop-filter: blur\(1px\)\)/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/app-shell.tsx"), "utf8"), /desk-canvas/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/exception-inbox.tsx"), "utf8"), /glass-panel workbench-filter-bar/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "app/page.tsx"), "utf8"), /btn btn-primary/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "app/board/page.tsx"), "utf8"), /className="card overflow-hidden"/);
  ...
  const compactShareCss = fs.readFileSync(path.join(process.cwd(), "app/globals.css"), "utf8");
...
`);
  ...
  assert.match(laneAvgCopy, /LANE_AVG_RADIUS_MILES = 200/);
  assert.match(laneAvgCopy, /LANE_AVG_MIN_SAMPLES = 3/);
  assert.match(laneAvgCopy, /parsePrintedLaneMiles/);
  assert.match(laneAvgCopy, /not enough lane history/);
  assert.match(laneAvgCopy, /laneEndsWithinRadius/);
  assert.match(laneAvgUi, /200-mi radius/);
  const laneAvgServer = fs.readFileSync(path.join(process.cwd(), "lib/lane-average.ts"), "utf8");
  assert.match(laneAvgServer, /laneEndsWithinRadius/);
  assert.match(laneAvgServer, /getResolvedFleetLaneRates/);
  assert.match(laneAvgServer, /laneAveragesForBoard/);
  assert.doesNotMatch(laneAvgServer, /laneKey\(row\.origin, row\.destination\) === key/);
  const boardLaneAvg = fs.readFileSync(path.join(process.cwd(), "app/board/page.tsx"), "utf8");
  assert.match(boardLaneAvg, /laneAveragesForBoard/);
  assert.doesNotMatch(boardLaneAvg, /listFleetLaneRates/);
  assert.doesNotMatch(boardLaneAvg, /laneAveragesForLoads/);
  const rateConImportMiles = fs.readFileSync(path.join(process.cwd(), "components/rate-con-import.tsx"), "utf8");
  assert.match(rateConImportMiles, /compareLaneAverage\(parsed\.rate, laneAverage, laneMiles\)/);
  assert.match(rateConImportMiles, /laneMiles=\{laneMiles\}/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "lib/actions.ts"), "utf8"), /parsePrintedLaneMiles/);
  const lumperRule = fs.readFileSync(path.join(process.cwd(), "lib/rate-con-fine-print-shared.ts"), "utf8");
  assert.doesNotMatch(lumperRule, /driver \(to \)\?\(pay\|unload\)/);
  assert.doesNotMatch(lumperRule, /chargebacks\?\|fine of/);
...
`;
  ...
  assert.match(fuelMatchUi, /FUEL_TABLE_PAGE_SIZE/);
  ...
  assert.match(fuelWeekUi, /data-fuel-spend="money"/);
  ...
  assert.match(fuelPage, /loadFuelWeekSpend/);
  assert.match(fuelPage, /fuelDeskMounts/);
  assert.match(fuelPage, /data-fuel-desk-panel/);
  assert.match(fuelPage, /weekView\.spent/);
  assert.doesNotMatch(fuelPage, /getSamsaraFleet/);
  const { fuelDeskMounts, resolveFuelDeskPanel, FUEL_TABLE_PAGE_SIZE } = await import("../lib/fuel-desk");
  assert.equal(resolveFuelDeskPanel({}), "spend");
  assert.equal(resolveFuelDeskPanel({ panel: "closeout", driverId: 4 }), "closeout");
  assert.equal(resolveFuelDeskPanel({ driverId: 4 }), "tx");
  assert.equal(resolveFuelDeskPanel({ view: "trucks" }), "tx");
  const spendMounts = fuelDeskMounts("spend");
  assert.equal(spendMounts.spend, true);
  assert.equal(spendMounts.import, true);
  assert.equal(spendMounts.weekStrip, true);
  assert.equal(spendMounts.closeout, false);
  assert.equal(spendMounts.mpg, false);
  assert.equal(spendMounts.receipts, false);
  assert.equal(spendMounts.audit, false);
  assert.equal(spendMounts.transactions, false);
  assert.equal(fuelDeskMounts("receipts").receipts, true);
  assert.equal(fuelDeskMounts("receipts").closeout, false);
  assert.equal(FUEL_TABLE_PAGE_SIZE, 25);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/fuel-desk-tabs.tsx"), "utf8"), /data-fuel-desk-tabs/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/fuel-desk-tabs.tsx"), "utf8"), /prefetch=\{value === "spend"/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/fuel-table-pager.tsx"), "utf8"), /data-fuel-table-pager/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "app/fuel/loading.tsx"), "utf8"), /data-fuel-loading/);
  const { fuelNavPaintBench } = await import("./fuel-nav-paint-bench");
  const fuelPaint = fuelNavPaintBench();
  assert.equal(fuelPaint.ok, true, "fuel first paint stays on the week spend panel");
  assert.ok(fuelPaint.light.bytes * 20 < fuelPaint.heavy.bytes, "a full fuel table is much larger than one page");
  ...
  const weekSpent = fuelWeekSpentTotalsForWeek(
    [
      ...
      { occurred_at: "2026-08-26T16:00:00.000Z", category: "cash advance", amount: 75 },
  ...
  assert.equal(weekSpent.money, 575);
  const priorWeekSpent = fuelWeekSpentTotalsForWeek(
    [
      ...
      { occurred_at: "2026-08-21T17:00:00.000Z", category: "money_code", amount: 30 },
    ],
    "2026-08-17",
  );
  ...
  const spendA = fuelStore.loadFuelWeekSpend("2026-08-24", wedNy);
  const spendB = fuelStore.loadFuelWeekSpend("2026-08-17", wedNy);
  assert.deepEqual(spendA.spent, weekA.spent);
  assert.deepEqual(spendA.stats, weekA.stats);
  assert.deepEqual(spendB.spent, weekB.spent);
  assert.deepEqual(spendB.stats, weekB.stats);
  assert.ok(spendA.weeks.some((week) => week.startYmd === weekA.weekStartYmd));
  assert.ok(spendB.weeks.some((week) => week.startYmd === weekB.weekStartYmd));
  ...
  const placeFixtures: Array<{ raw: string; cityState: string; chopped: RegExp }> = [
    { raw: "400 N Burlington Ave, Hastings, NE 68901", cityState: "Hastings, NE", chopped: /^STINGS|^stings/i },
    ...
    { raw: "Hastings, NE", cityState: "Hastings, NE", chopped: /^STINGS|^stings/i },
    ...
    { raw: "South Sioux City, NE", cityState: "South Sioux City, NE", chopped: /^UTH SIOUX|^uth sioux/i },
    { raw: "500 MAIN ST, HOLCOMB, KS 67851", cityState: "HOLCOMB, KS", chopped: /^LCOMB/ },
    { raw: "HOLCOMB, KS", cityState: "HOLCOMB, KS", chopped: /^LCOMB/ },
    { raw: "Holcomb, KS", cityState: "Holcomb, KS", chopped: /^LCOMB|^lcomb/i },
    ...
    { raw: "Dock 4, 1800 E 8th Ave, Lakewood, NJ, 08701, US", cityState: "Lakewood, NJ", chopped: /^KEWOOD|^kewood/i },
  ];
  ...
  assert.match(boardCss, /Phone Board stack \(~390px\)/);
  const phoneBoardCss = boardCss.match(/Phone Board stack \(~390px\)[\s\S]*?Orbcomm phone:/)?.[0] ?? "";
  assert.match(phoneBoardCss, /position:\s*static/);
  assert.match(phoneBoardCss, /grid-area:\s*status/);
  assert.match(phoneBoardCss, /grid-area:\s*end/);
  assert.match(phoneBoardCss, /grid-area:\s*pickup/);
  assert.match(phoneBoardCss, /content:\s*"Pickup"/);
  assert.match(phoneBoardCss, /\.board-delivery-cell[\s\S]*display:\s*none/);
  assert.doesNotMatch(phoneBoardCss, /width:\s*9rem/);
  assert.doesNotMatch(phoneBoardCss, /left:\s*5rem/);
  assert.doesNotMatch(phoneBoardCss, /max-width:\s*5rem/);
  const phoneTabCss = boardCss.match(/After base chip padding so phone compact wins[\s\S]*?white-space:\s*nowrap;/)?.[0] ?? "";
  assert.match(phoneTabCss, /flex-wrap:\s*nowrap/);
  assert.match(phoneTabCss, /overflow-x:\s*auto/);
  assert.match(phoneTabCss, /padding:\s*0\.12rem 0\.38rem/);
  assert.match(phoneTabCss, /font-size:\s*11px/);
  assert.match(phoneTabCss, /white-space:\s*nowrap/);
  assert.match(boardPage, /board-pickup-cell/);
  assert.match(boardPage, /data-board-phone-stack/);
  assert.match(boardPage, /data-board-status/);
  assert.match(boardPage, /data-board-end/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/board-toolbar.tsx"), "utf8"), /load-tab-short/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "lib/load-list-shared.ts"), "utf8"), /short: "Accounting"/);
...
`);
  ...
  assert.equal(backhaulShared.isHouseCustomerName("M and S Loads"), true);
  assert.equal(backhaulShared.isHouseCustomerName("MNS Loads"), true);
  assert.equal(backhaulShared.isHouseCustomerName("MS Loads"), true);
  assert.equal(backhaulShared.isHouseCustomerName("msloadsllc"), true);
  assert.equal(backhaulShared.isHouseCustomerName("MSLOADS"), true);
  assert.equal(backhaulShared.isHouseCustomerName("River City Produce"), false);
  assert.equal(backhaulShared.isHouseCustomerName("Heartland Foods Co."), false);
  assert.ok(findCityCenter("Bronx, NY"), "Bronx must geocode from the public city table");
  ...
  const riverCustomerId = queries.findOrCreateCustomer("River City Produce");
  ...
  function backhaulLaneLoad(input: {
    ...
    return queries.createLoad({
      ...
      pickup_end: input.pickup_start,
      delivery_start: input.delivery_start,
      delivery_end: input.delivery_start,
      weight: 40000,
      commodity: "Produce",
      rate: 1800,
      notes: "",
  ...
  const sourceId = backhaulLaneLoad({
    ...
    origin: "Chicago, IL",
    destination: "Bronx, NY",
    pickup_start: "2026-09-01T12:00:00.000Z",
  ...
  const nearId = backhaulLaneLoad({
    ...
    origin: "Newark, NJ",
    destination: "Bronx, NY",
    pickup_start: "2026-08-12T12:00:00.000Z",
  ...
  const midId = backhaulLaneLoad({
    ...
    pickup_start: "2026-07-02T12:00:00.000Z",
  ...
  const closestId = backhaulLaneLoad({
    ...
    origin: "Bronx, NY",
    destination: "Chicago, IL",
    pickup_start: "2026-06-18T12:00:00.000Z",
  ...
  const houseNearId = backhaulLaneLoad({
    ...
    origin: "Newark, NJ",
    destination: "Bronx, NY",
    pickup_start: "2026-08-01T12:00:00.000Z",
    delivery_start: "2026-08-01T18:00:00.000Z",
  });
  const cancelledId = queries.createLoad({
    load_number: "MSE-BH99",
    customer_id: westsideId,
    origin: "Newark, NJ",
    destination: "Bronx, NY",
    pickup_start: "2026-08-01T12:00:00.000Z",
    pickup_end: "2026-08-01T12:00:00.000Z",
    delivery_start: "2026-08-01T18:00:00.000Z",
    delivery_end: "2026-08-01T18:00:00.000Z",
    weight: 40000,
    commodity: "Produce",
    rate: 1800,
    notes: "",
  ...
  assert.equal(
    ...
    false,
    "house accounts must not appear in the customer rollup",
  );
  assert.ok(found.customers.some((row) => row.name === "Westside Foods"));
  assert.ok(found.customers[0] && found.customers[0].loads >= found.customers[found.customers.length - 1].loads);
  const missing = await backhaul.findBackhaulForLoad(missingId);
  ...
  stopsMod.ensureDefaultStops(emptyCenter);
  ...
  const {
    compareLaneAverage,
    LANE_AVG_MIN_SAMPLES,
    LANE_AVG_RADIUS_MILES,
    laneAvgShouldShow,
    laneEndMiles,
    ...
    laneLabel,
    lanePointFromText,
    parsePrintedLaneMiles,
    resolveLanePoint,
  } = await import("../lib/lane-average-shared");
  const { scanRateConFinePrint } = await import("../lib/rate-con-fine-print-shared");
  const { laneAverageForLoad, laneAverageMatchIds, laneAverageSnapshot, laneAveragesForBoard } =
    await import("../lib/lane-average");
  assert.equal(LANE_AVG_RADIUS_MILES, 200);
  assert.equal(LANE_AVG_MIN_SAMPLES, 3);
  assert.equal(parsePrintedLaneMiles("Rate $2,400. Miles: 1,200. Weight 40,000"), 1200);
  assert.equal(parsePrintedLaneMiles("Loaded 880 miles. Rate $1800."), 880);
  assert.equal(parsePrintedLaneMiles("Rate $2400. Weight 40000 lbs."), null);
  ...
  const dodge = resolveLanePoint("Dodge City, KS");
  const holcomb = resolveLanePoint("Holcomb, KS");
  const cedarRapids = resolveLanePoint("Cedar Rapids, IA");
  const davenport = resolveLanePoint("Davenport, IA");
  ...
  const laneHist = (
    ...
    destination: string,
    rate: number,
    status: "completed" | "delivered" | "cancelled" | "available" = "completed",
    extra: { shipper_location_id?: number } = {},
  ) =>
    queries.createLoad({
      customer_id: customerId,
      origin,
  ...
  const nearbyHist = laneHist("Holcomb, KS", "Davenport, IA", 2400);
  const oneSample = laneAverageSnapshot({
    origin: "Dodge City, KS",
    destination: "Cedar Rapids, IA",
  });
  assert.equal(oneSample?.sampleSize, 1);
  assert.equal(laneAvgShouldShow(compareLaneAverage(1800, oneSample, 1200)), false, "do not badge on n=1");
  assert.equal(compareLaneAverage(1800, oneSample).band, "none");
  const exactHist = laneHist("Dodge City, KS", "Cedar Rapids, IA", 2000, "delivered");
  const twoSample = laneAverageSnapshot({
    origin: "Dodge City, KS",
    destination: "Cedar Rapids, IA",
  });
  assert.equal(twoSample?.sampleSize, 2);
  assert.equal(laneAvgShouldShow(compareLaneAverage(1800, twoSample)), false, "n=2 is still below min samples");
  const thirdHist = laneHist("Dodge City, KS", "Davenport, IA", 2200);
  const farDestHist = laneHist("Holcomb, KS", "Houston, TX", 5000);
  const farPickupHist = laneHist("Chicago, IL", "Davenport, IA", 5100);
  const farBothHist = laneHist("Phoenix, AZ", "Houston, TX", 5200);
  const laneCancelled = laneHist("Holcomb, KS", "Cedar Rapids, IA", 900, "cancelled");
  getDb().prepare("UPDATE loads SET route_miles = 1200 WHERE id IN (?, ?, ?)").run(nearbyHist, exactHist, thirdHist);
  const liveLane = laneHist("Dodge City, KS", "Cedar Rapids, IA", 1800, "available");
  ...
  assert.ok(laneSnap);
  assert.equal(laneSnap.sampleSize, 3, "nearby + exact + third in; far dest/PU/both out; cancelled out");
  assert.equal(laneSnap.avgRate, 2200);
  assert.equal(laneSnap.avgPerMile, 1.83);
  const matchIds = laneAverageMatchIds({
    origin: "Dodge City, KS",
    destination: "Cedar Rapids, IA",
    excludeLoadId: liveLane,
  });
  assert.ok(matchIds?.includes(nearbyHist), "Holcomb→Davenport is inside 200-mi both ends");
  assert.ok(matchIds?.includes(exactHist), "exact Dodge→Cedar counts");
  assert.ok(matchIds?.includes(thirdHist), "Dodge→Davenport dest is inside Cedar Rapids 200-mi");
  assert.equal(matchIds?.includes(farDestHist), false, "far dest is out even if PU were close");
  assert.equal(matchIds?.includes(farPickupHist), false, "far PU is out even if dest were close");
  assert.equal(matchIds?.includes(farBothHist), false, "far both ends is out");
  assert.equal(matchIds?.includes(laneCancelled), false, "cancelled never joins the history pool");
  assert.equal(matchIds?.includes(liveLane), false);
  const below = laneAverageForLoad(queries.getLoad(liveLane)!);
  assert.equal(below.band, "below");
  assert.equal(below.sampleSize, 3);
  assert.match(String(below.perMile), /1\.5/);
  assert.equal(laneAvgShouldShow(below), true);
  ...
  assert.equal(laneAvgShouldShow(none), false, "empty / ungeocoded set hides the badge");
  const lonely = compareLaneAverage(
    1800,
    laneAverageSnapshot({ origin: "Minneapolis, MN", destination: "Detroit, MI" }),
  );
  assert.equal(lonely.sampleSize, 0, "geocoded city with no radius history is empty");
  assert.equal(laneAvgShouldShow(lonely), false);
  const boardMap = laneAveragesForBoard([
    queries.getLoad(liveLane)!,
    queries.getLoad(farBothHist)!,
    queries.getLoad(laneCancelled)!,
  ]);
  assert.equal(boardMap.get(liveLane)?.band, "below");
  assert.equal(boardMap.get(farBothHist)?.sampleSize, 0, "Phoenix→Houston is outside the Dodge→Cedar 200-mi radius");
  assert.equal(
    boardMap.get(laneCancelled)?.sampleSize,
    4,
    "cancelled Holcomb→Cedar still sees live + nearby + exact + third",
  );
  const cimarronYard = queries.createLocation({
    name: "Cimarron Yard",
    street: "100 County Rd",
    city: "Cimarron",
    state: "KS",
    zip: "67835",
    phone: "",
  ...
  const storedPickup = laneHist("Cimarron, KS", "Davenport, IA", 2100, "completed", {
    shipper_location_id: cimarronYard,
  });
  getDb().prepare("UPDATE loads SET route_miles = 1200 WHERE id = ?").run(storedPickup);
  const afterStored = laneAverageSnapshot({
    origin: "Dodge City, KS",
    destination: "Cedar Rapids, IA",
    excludeLoadId: liveLane,
  });
  assert.equal(
    afterStored?.sampleSize,
    4,
    "Cimarron stored lat/lng is inside Dodge 200-mi even though Cimarron is not a catalog city",
  );
  assert.ok(queries.getLoad(storedPickup));
  ...
  const lumperFalse = scanRateConFinePrint(
    "Driver will unload at the dock. Driver pay is via settlement. Rate $2400.",
  );
  assert.equal(lumperFalse.some((hit) => hit.kind === "lumper"), false, "bare driver unload/pay is not lumper");
  assert.equal(lumperFalse.some((hit) => hit.kind === "penalty"), false, "a dollar rate is not a penalty");
  const chargebackFalse = scanRateConFinePrint("Chargebacks apply to all invoices.");
  assert.equal(chargebackFalse.some((hit) => hit.kind === "penalty"), false, "bare chargeback is not a penalty");
  const fineForm = new FormData();
  fineForm.set(
    "rate_con",
    new File(
      ["Detention $60/hr after free time. TONU $200. Carrier pays lumper. Late fee $80. Miles: 1200."],
      "money-terms.txt",
      { type: "text/plain" },
    ),
  );
