...
async function main() {
  ...
  assert.doesNotMatch(basicsChunk, /htmlFor="rate"|Customer pay|Customer rate|name="oo_percent"|name="oo_pay"/);
  assert.doesNotMatch(basicsChunk, /data-oo-percent|data-oo-pay|data-oo-pay-pair|impliedOwnerOperatorPercent/);
  const payFields = fs.readFileSync(path.join(process.cwd(), "components/load-customer-pay.tsx"), "utf8");
  assert.match(payFields, /data-customer-pay/);
  assert.match(payFields, /Customer pay/);
  assert.match(payFields, /htmlFor="rate"/);
  assert.match(payFields, /name="oo_percent"/);
  assert.match(payFields, /name="oo_pay"/);
  assert.match(payFields, /data-oo-percent/);
  assert.match(payFields, /data-oo-pay/);
  assert.match(payFields, /data-oo-pay-pair/);
  assert.match(payFields, /impliedOwnerOperatorPercent/);
  assert.match(payFields, /Dollars/);
  assert.match(payFields, /Percent of flat rate/);
  assert.match(payFields, /data-critical-save/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/load-form.tsx"), "utf8"), /load\?\.oo_percent \?\? driver\.pay_percent/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/load-form.tsx"), "utf8"), /LoadCustomerPay/);
  assert.match(fs.readFileSync(path.join(process.cwd(), "components/load-form.tsx"), "utf8"), /data-load-screen="financials"/);
  ...
  assert.match(financialsTab, /LoadMoneyBox/);
  ...
  assert.match(docsPage, /when=\{\["basics", "customer", "assets", "financials"\]\}/);
