...
- Trailers: unit #, type, VIN/plate, assigned truck, registration + DOT, ORBCOMM id, default reefer setpoint, last known reading stub, last GPS, notes, active flag, document uploads. **Import from ORBCOMM** uses `ORBCOMM_*` when the API returns assets, or a CSV/export preview-then-import. Accepts Location Tracking Report / Reefer Status Report (skips title/date banners). Columns are matched loosely: Trailer # / Asset Name / Unit / Vehicle, Asset ID / Device ID / Mobile ID, VIN, Lat/Lng, City/Address. Preview shows trailer #, asset id, VIN, last city/lat-lng. Import writes GPS onto the trailer. Match by asset id or trailer #. No portal scrape.
