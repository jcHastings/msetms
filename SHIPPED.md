...
- **Tolls** in Fleet nav — Office-only PrePass tolls tab mirrors Fuel flow: CSV/XLSX import, transponder-to-truck rematch (`trucks.prepass_transponder_id`), manual assign queue, per-driver and per-transaction views, weekly picker with monthly rollup toggle, and fleet/driver totals for toll + scale-bypass categories. Stores both category types when present. `PREPASS_API_KEY` enables a thin API-pull stub; when missing (or API docs remain unresolved) pull no-ops with a clear message and manual import still works.
