...
- Trucks: unit #, year/make/model, plate, VIN, assigned driver, registration + DOT dates, Samsara vehicle id, notes, active flag, document uploads. **Import from Samsara** (preview name/unit, vehicle id, VIN → confirm) creates/updates by Samsara vehicle id or unit #. No duplicates. Missing token: add it in `.env`. Token never logged.
- Trailers: unit #, type, VIN/plate, assigned truck, registration + DOT, ORBCOMM id, default reefer setpoint, last known reading stub, notes, active flag, document uploads. **Import from ORBCOMM** uses `ORBCOMM_*` when the API returns assets, or a CSV/export preview-then-import. Match by asset id or trailer #. No portal scrape.
...
- **Mike** on every dispatcher page (header + bottom-right button), not the driver app. The panel always opens. If `OPENAI_API_KEY` is set (including `sk-` keys loaded from project `.env`, `process.cwd()/.env`, or `.next/standalone/.env`), chat works; if not, Mike says add the key to `.env` and restart. Cheap `gpt-4o-mini`. Never invents GPS. Never logs the key or PINs.
