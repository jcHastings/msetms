...
- Driver confirmation maps broker stop Notes and operational Directions, keeps Purchase Order # separate from Confirmation number, prints per-stop cases, set-appointment flags, and the full load-level pulp/temp/load-lock notes. The customer/broker load number stays off the driver sheet. Legal boilerplate is not copied.
...
- Rate-con contact extraction reads the Name | Phone (xEXT) | Email | Fax row on the driver/carrier sheet, including when that table sits above the “CONTACT INFO” title. CARRIER CONTACT, shipper/receiver phones in stop notes, and “send POD to” billing lines are ignored. Other brokers use their own labels. Values come from that packet and that customer only — no assumed broker, domain, or phone. If a field is missing, leave it blank. Do not invent.
