...
- Driver app unchanged: `/driver/login` with seeded driver PINs. No TOTP. When a load has an assigned trailer, **Trailer** is a tab. It opens a pinch-zoom map with a small teardrop pin at that trailer’s last-known Orbcomm location. No trailer assigned? The tab stays hidden. Tracking maps office-wide use the same small pin (Orbcomm/Samsara colors stay on the pin head). Positions are never invented.
