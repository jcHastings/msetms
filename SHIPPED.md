...
- Trucks: unit #, year/make/model, plate, VIN, assigned driver, registration + DOT dates, Samsara vehicle id, notes, active flag, document uploads. **Import from Samsara** matches vehicle id, name, or unit # (JC’s unit **36**, or a typed unit, is enough — no UUID required) and writes the real Samsara vehicle id. If the API returns vehicles but not that unit, the preview lists the names that came back. Live GPS + HOS on the truck list, truck detail, dispatch board, dashboard On the road, and load Carrier/Asset when that truck is assigned. Empty: “No Samsara ID on this truck — Import from Samsara or paste the vehicle id.” Positions are never invented. Missing token: add it in `.env`. Token never logged.
