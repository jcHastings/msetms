...
- **Fuel** in Fleet nav — daily fuel-card CSV or Transaction Activity Report PDF (`unpdf`). Four first-class buckets on every import and total: Truck diesel, Reefer diesel, DEF, Scale (none of these is “Other”; DEF is never dumped into truck diesel). Match by driver name (including NName last, first) or unit / prompt #. Dedup on invoice + category + qty when invoice is present. Per-driver and per-truck totals. Unmatched queue. Template + export. Driver and truck pages link here. No live card API. **Weekly fuel closeout** on Fuel + Mike (end-of-week): Samsara odometer miles, TMS diesel / line items, MPG, audit flags, green lights, fleet rollup, Markdown/HTML filed for CoS. Draft to JC only. Nothing sent to drivers.
- **Tolls** in Fleet nav — Office-only PrePass tolls tab mirrors Fuel flow: CSV/XLSX import, transponder-to-truck rematch (`trucks.prepass_transponder_id`), manual assign queue, per-driver and per-transaction views, weekly picker with monthly rollup toggle, and fleet/driver totals for toll + scale-bypass categories. Stores both category types when present. `prepass_transponder_id` may be blank and does not block readiness; unmapped rows stay unassigned until mapping/manual assign. `PREPASS_CLIENT_ID` + `PREPASS_CLIENT_SECRET` + `PREPASS_ACCOUNT_NUMBER` pull posted tolls from PrePass REST (`/auth/v1/token`, `/tolltransaction/v1/transactions`, last 14 posted days). Missing OAuth or account number shows a clear `/tolls` banner and leaves CSV/XLSX working. Do not invent an account number. Legacy `PREPASS_API_KEY` is not enough for REST pull. Secrets and tokens are never logged.
...
- Rate-con contact extraction reads the Name | Phone (xEXT) | Email | Fax row on the driver/carrier sheet, including when that table sits above the “CONTACT INFO” title. CARRIER CONTACT, the Carrier / Attn line (the trucking company on a broker sheet), shipper/receiver phones in stop notes, and “send POD to” billing lines are ignored. Contact name is a person only — never city/state/zip, street, or the load number. If the packet has no broker dispatcher, Contact name stays blank and the header phone still fills Per-load phone. Other brokers use their own labels. Values come from that packet and that customer only — no assumed broker, domain, or phone. If a field is missing, leave it blank. Do not invent.
