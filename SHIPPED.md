...
- Rate-con contact extraction reads the Name | Phone (xEXT) | Email | Fax row on the driver/carrier sheet, including when that table sits above the “CONTACT INFO” title. CARRIER CONTACT, the Carrier / Attn line (the trucking company on a broker sheet), shipper/receiver phones in stop notes, and “send POD to” billing lines are ignored. The per-load contact is the broker on that rate conf, not our Attn name or office phone. Other brokers use their own labels. Values come from that packet and that customer only — no assumed broker, domain, or phone. If a field is missing, leave it blank. Do not invent.
