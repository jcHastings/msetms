...
- **Load map** (Load Actions → Load map): this load only — pickup/delivery stops, assigned Samsara tractor GPS, stored tracking pins, and ORBCOMM trailer GPS when lat/lng already exists. Uses a browser key via `GOOGLE_MAPS_BROWSER_KEY` (enable Maps JavaScript API; keep this key referrer-restricted). Server Places/Directions still use `GOOGLE_MAPS_API_KEY` (or `GOOGLE_PLACES_API_KEY`). The map hides if the browser key or points are missing; addresses still save by hand. Coordinates are never invented. Not a fleet map. Official IFTA stays Samsara.
...
- **Compliance** hub (`/compliance`): Overview embeds the existing expiry list (`listUpcomingCompliance`) and hosts **Drug & alcohol** CRUD. Failed tests reuse the danger badge / Workbench list — not a second expiry board. Driver detail shows the last 5 tests.
