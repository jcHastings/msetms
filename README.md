...
- Dashboard counts, shift handoff, watch list, daily recap, dispatch board with status / pickup-date filters. **Mike** is a side chat on the board (not the driver app). Add `OPENAI_API_KEY` only — Mike uses cheap `gpt-4o-mini`. Answers from TMS data only and will not invent GPS.
