...
- Tractor GPS and driver HOS from **Samsara** (live when `SAMSARA_API_TOKEN` is set). Shown on **Trucks** (list + detail), the dispatch board, dashboard **On the road**, and the load **Carrier / Asset** tab when that truck is assigned. Match on Samsara vehicle id, name, or unit # (including JC’s unit **36**, or a unit typed as the vehicle id). You do not need the UUID. Import fills the real Samsara vehicle id. If the API returns vehicles but not that unit, the preview lists the names that came back. Missing ID: “No Samsara ID on this truck — Import from Samsara or paste the vehicle id.” Positions are never invented. Missing token: add `SAMSARA_API_TOKEN` to `.env` and restart. The token is never logged.
